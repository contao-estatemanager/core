 ...
 */
class EstateManagerController extends AbstractEstateManagerController
{
    /**
     * Method
     * @var string
    ...
    /**
     * Runs the command scheduler. (estatemanager)
     *
     * @Route("/api/estatemanager/v{version}/{module}/{id}", name="estatemanager_run")
     *
     * @return JsonResponse
     */
    public function run($version, $module, $id=null)
    {
        $data = array(
            'results' => null,
        ...
        switch ($module)
        {
            case 'estates':
                ...
                // prepare module
                if ($this->currParam['moduleId'] ?? null)
                {
                    $this->objModule = ModuleModel::findByPk($this->currParam['moduleId']);
                }

                if(($this->currParam['filter'] ?? null) && ($this->currParam['filterMode'] ?? null))
                {
                ...
                switch($this->currParam['dataType'] ?? null)
                {
                    case 'geojson':
            ...
            case 'contactpersons':
                ...
                if(\is_array($this->currParam['fields'] ?? null))
                {
                        ...
                        foreach ($this->currParam['fields'] as $field)
                        {
                            switch($field)
                            {
                                ...
                                case 'singleSRC':
                                    ...
                                    if(\is_array($this->currParam['imgSize'] ?? null))
                                    {
        ...
        if($data['type'] !== null || ($data['results'] !== null && \count($data['results'])))
        {
