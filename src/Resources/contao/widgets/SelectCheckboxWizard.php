 ...
 */
class SelectCheckboxWizard extends Widget
{
	/**
	 * Submit user input
	 * @var boolean
	...
	public function validate()
	{
		...
		foreach ($varValue as &$v) {
			...
			// Save empty checkbox as 0
			if(!isset($v[$this->arrFieldNames[1]]))
			{
				$v[$this->arrFieldNames[1]] = '0';
			}
		}
	...
	public function generate()
	{
		...
		for ($i=0, $c=\count($this->varValue); $i<$c; $i++)
		{
			...
			// Build input field
			$strFields .= vsprintf('<input type="checkbox" name="%s" id="%s" class="%s" value="1"%s onfocus="Backend.getScrollOffset()"></td><td>', [
				$this->strId . '['.$i.']['.$this->arrFieldNames[1].']',
				$this->strId . '['.$i.']['.$this->arrFieldNames[1].']',
				'tl_checkbox',
				!!$this->varValue[$i][$this->arrFieldNames[1]] ? ' checked' : ''
			]);
		...
		return vsprintf(
			'<table id="ctrl_%s" class="%s">%s<tbody class="sortable">%s</tbody></table>%s', [
			$this->strId,
			'tl_modulewizard cem_selectCheckboxWizard',
			$strLabels,
			$strFields,
			'<script>CemBackend.UniversalWizard("ctrl_' . $this->strId . '")</script>'
		]);
	}
}
